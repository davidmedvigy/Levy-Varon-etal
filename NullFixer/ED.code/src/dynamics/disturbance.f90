...
module disturbance_utils
   ...
   subroutine apply_disturbances(cgrid)
     use phenology_startup, only: initialize_paw
      ...
      real :: plantN1, soilN1, totalN1, plantN2, soilN2,totalN2
      ...
      plantN1=0.;plantN2=0.;soilN1=0.;soilN2=0.;totalN1=0.;totalN2=0.
      polyloop: do ipy = 1,cgrid%npolygons
         ...
         siteloop: do isi = 1,cpoly%nsites
            ...
            do ipa = 1, csite%npatches
               ...
               soilN1 = soilN1 + (csite%fast_soil_N(ipa) + csite%structural_soil_C(ipa)/c2n_structural + csite%mineralized_soil_N(ipa)) &
                         * csite%area(ipa)             

               do ico = 1, cpatch%ncohorts
                  plantN1 = plantN1 + csite%area(ipa) * cpatch%nplant(ico) * (cpatch%balive(ico)/c2n_leaf(cpatch%pft(ico)) &
                           + cpatch%bdead(ico)/c2n_structural + cpatch%nstorage(ico)) !JL!
               enddo
            enddo
...
print*,'NITROGREN, INIT',soilN1,plantN1,soilN1+plantN1
            ...
            new_lu_loop: do new_lu = 1, n_dist_types
	          ...
	          csite%dist_fast_soil_N(1:(onsp+new_lu))       = 0     !JL!
    	       	  csite%dist_structural_soil_C(1:(onsp+new_lu)) = 0     !JL!
                  ...
                  call initialize_paw(csite,new_lu+onsp,cpoly%ntext_soil(:,isi))
            ...
            do ipa = 1, csite%npatches
               ...
               soilN2 = soilN2 + (csite%fast_soil_N(ipa) + csite%structural_soil_C(ipa)/c2n_structural + csite%mineralized_soil_N(ipa)) * csite%area(ipa)
                    

               do ico = 1, cpatch%ncohorts
                  plantN2 = plantN2 + csite%area(ipa) * cpatch%nplant(ico) * (cpatch%balive(ico)/c2n_leaf(cpatch%pft(ico)) + &
                           cpatch%bdead(ico)/c2n_structural +cpatch%nstorage(ico)) !JL!
               enddo
            enddo

print*,'NITROGREN, FINAL',soilN2,plantN2,soilN2+plantN2
   ...
   subroutine initialize_disturbed_patch(csite,atm_tmp,np,dp,lsl)
      ...
      csite%dist_fast_soil_N           (np) = 0.0 !JL
      csite%dist_structural_soil_C     (np) = 0.0 !JL
      csite%interval_DON_loss          (np) = 0.0 !JL
   ...
   subroutine increment_patch_vars(csite,np, cp, area_fac)
      ...
      csite%fast_soil_N                (np) = csite%fast_soil_N                (np)        &
                                            ...
                                            * area_fac
      !______JL____________________________________! 
     csite%dist_fast_soil_N           (np) = csite%dist_fast_soil_N           (np)        &
                                            + csite%dist_fast_soil_N           (cp)        &
                                            * area_fac				

     csite%dist_structural_soil_C      (np) = csite%dist_structural_soil_C     (np)        &
                                            + csite%dist_structural_soil_C     (cp)        &
                                            * area_fac				    
     csite%interval_DON_loss           (np) = csite%interval_DON_loss          (np)        &
                                            + csite%interval_DON_loss          (cp)        &
                                            * area_fac  
      !______JL____________________________________!
   ...
   subroutine accum_dist_litt(csite,np,cp,q,area_fac,loss_fraction,poly_dest_type          &
      ...
      csite%dist_fast_soil_N(np)  = csite%dist_fast_soil_N(np)  + fast_litter_n * area_fac            !JL!
      csite%dist_structural_soil_C(np)  = csite%dist_structural_soil_C(np) + struct_litter * area_fac !JL!
