...
subroutine resp_rh(csite,ipa,Lc)
  ...
  !write(*,*) 'slow soil c',csite%slow_soil_C(ipa),'slow c loss',slow_C_loss,'Difference', &
  ! (csite%slow_soil_C(ipa)- slow_C_loss) !JL! 
   ...
   polygonloop: do ipy = 1,cgrid%npolygons
      ...
      siteloop: do isi = 1,cpoly%nsites
         ...
         patchloop: do ipa = 1,csite%npatches
            ...
            ! Checks
            !print*,'r_stsc', r_stsc
            !print*, 'struct and slow N LOSS',Struct_slow_N_loss,' slow_C_loss/c2n_slow', &
            ! slow_C_loss / c2n_slow,'structural_C_loss/c2n_structural',                  & 
            !structural_C_loss/c2n_structural
            ...
            !N_deposition = 0.0005/365 + 0.0004/365  !(kg N/m2/day )
   
            !------------------------------------------------------------------------------! 
            !------------------------------------------------------------------------------! 
            !Starting to consider BNF_input based on calculations from other modeles.      !
            ! CALCULATION 1: Schmiel 1996, based on annual evapotranspiration              !
            ! CALCULATION 2: Thornton 2007, based on annual NPP                            !
            ! CALCULATION 3: Zaehle 2010, based on evapotranspiraiotn from cleveland (1999)!
            !--------------------------------------------------------------- --------------!
! specify which calculation to use for BNF input
            calculation = 1
 if( calculation == 1) then
            !CALCULATION 1: Schmiel 1996, based on annual evapotranspiration
            BNF_input_today =0

            !evapotranspiration (kg water/m2/year)  ,  cgrid%annual_evapotranspiration(1)  
            !print*,'SR',' cgrid%total_evapotranspiration(1)' cgrid%total_evapotranspiration(1)
           ...
           ! Convert evapotranspiraiton to units  used in Schmiel 1996 (cm/yr)
            ...
            BNF_input = max(0.0, 0.008* (evapotranspiration_converted - 40))! Fixation UNITS UNKNOWN  ..  
           
            !MAY NEED TO Convert BNF input to daily timestep depending on the timestep Schmiel 1996 calculated BNF on
            ! Need units for BNF to convert N input to to KG N/m2!!
            ! Best guess is that the units are gN/m2 because these are the units for all other soil N parameters in the model
            ! the timeframe is very uncertain. This could be a value for the yearly input or the daily input. I will see what the model ouput looks             !like and adjust accordingly
           ! BNF_input_today = BNF_input *0.001 ! kg N/m2 , time? this assumes daily timestep output
           BNF_input_today = BNF_input/30 *0.001 ! kg N/m2 , time? this assumes monthly timestep output
           ! BNF_input_today = BNF_input/365 *0.001 ! kg N/m2 , time? this assumes annual timestep output
            
            !The paper says it adds BNF constantly so adding it at a daily scale might be reasonalbe... 
            !------------------------------------------------------------------------------! 
 elseif( calculation == 2) then
            ! CALCULATION 2: Thornton 2007, based on annual NPP 
            
             ! convert annual NPP to Thornton units[g C/m2/yr].cgrid%annual_NPP(1) is in kg C/m2/yr
             !1000g =1kg
            BNF_input_today =0              
            npp_converted =  cgrid%annual_NPP(1)*1000   ![g C/m2/yr]
            BNF_input = 1.8 * (1.0 - exp(-0.003 * npp_converted)) ![g N/m2/yr]
            BNF_input_today = (BNF_input/1000)/365 ![kg N/m2/day] 
             !------------------------------------------------------------------------------!           
 elseif( calculation == 3) then
             !CALCULATION 3: Zaehle 2010, based on evapotranspiraiotn from cleveland (1999)!
                         !AAET is the 30-year mean annual actual evapotranspiration (in mm a-1), 
  ! I decided to use annual total because the forest is changing so much in the first 80 years,
  ! it is not at a steady state to use a mean
            !evapotranspiration  (kg water/m2/year)  
             AAET= cgrid%annual_evapotranspiration(1)   ! [ kg H20/m2/year] want it in [mm/yr]
             evapotranspiration_converted=AAET*1/water_density*1000 ! gives mm/yr
          
             BNF_input = 0.1 * max((0.0234 *evapotranspiration_converted + 0.172), 0.0) ! [ g N/m2/yr]
             BNF_input_today = BNF_input*1/1000*1/365 ![kg N/m2/day] 
             !  BNF is set to zero if the sum of soil NH4 and NO3 exceeds 2 g N m􀀀2 
             
             ! CONCERN , this equation is not the same as cleveland et al, the source of the equation. 
             !Clevelands equation is   BNF = 0.234*(evapotranspiration) - 0.172
             !However they do hint that they modified the equation, but it is not clear 

             !ALSO: BNF is set to zero if the sum of soil NH4 and NO3 exceeds 2 g N m􀀀2 
             ! This would mean that above 0.002 kg N/m2 BNF could not occur. IN our model we sart at 
             ! 0.0017 kg N/m2 for the minearlized soil N pool and it increases rapidly.. so BNF would
             ! never occur.Omitting this condition for now. 

            !------------------------------------------------------------------------------! 
 elseif( calculation == 4) then
            ! CALCULATION 2: Using cleveland 1999's relationship directly 
             water_density = 1000    !1000kg water/m3 water 
            evapotranspiration_converted= cgrid%annual_evapotranspiration(1)*1/water_density*100 ! gives cm/yr
            BNF_input = max(0.0,0.234*evapotranspiration_converted - 0.172) ! Fixation [kg N/ha/yr]
            BNF_input_today = BNF_input * 1/365 * 1/10000 ![kg N/m2/day] 
            !------------------------------------------------------------------------------!           
            ...
            !Avg_soil_water = sum(csite%soil_water(:nzg,ipa))/14
...
!water_input = 1
!print*, 'water input', water_input
!print*, 'cpoly%daily_pcpg(1) * sec_in_day',cpoly%daily_pcpg(1) * sec_in_day ,&
!' csite%daily_transp(ipa) * sec_in_day', cpoly%daily_transp(ipa) * sec_in_day,&
!' csite%daily_evap(ipa) * sec_in_day  ', cpoly%daily_evap(ipa) * sec_in_day  ,&
!'cpoly%daily_pcpg(1)',cpoly%daily_pcpg(1),                                    &
!'csite%daily_transp(ipa)',cpoly%daily_transp(ipa),                            &
!'csite%daily_evap(ipa)',cpoly%daily_evap(ipa)
            ...
            !print*,' total_soil_water',total_soil_water,' total_field_capacity',          &
            !total_field_capacity

            !DON loss is 1/5 of the total N deposition
             DON_loss    = 0.0001/365 !JL
 
            !print*, 'START DON', '   DON_loss',DON_loss, 'structural_N_loss',             &
            ! structural_N_loss,'csite%interval_DON_loss(ipa)',                            &
            !csite%interval_DON_loss(ipa),'DON_leached ',DON_leached
            ...
            !print*, 'END DON', '   DON_loss',DON_loss, 'structural_N_loss',               &
            !structural_N_loss,'csite%interval_DON_loss(ipa)',csite%interval_DON_loss(ipa),&
...
! note, Because BNF and evapotranspiration are based on cgrid parameters that give a forest average, the
! annual ouput  is the same for each patch and therefore does not need to be scaled by area in the 
! calculations below
...
!print*, 'total_don_loss' ,cgrid%total_DON_loss(1),' DON_leached', DON_leached,'csite%area(ipa)', csite%area(ipa), ' cgrid%total_N_Fixation(1)',cgrid%total_N_Fixation(1)
            !-------------------------------------------------------------------------------!
            !----- Mineralized pool. -------------------------------------------------------!

            csite%mineralized_N_input(ipa) = fast_N_loss + slow_C_loss / c2n_slow           & 
                                            +  structural_N_loss                            & 
                                            + N_deposition  + BNF_input_today                             

            csite%mineralized_N_loss(ipa)  = csite%total_plant_nitrogen_uptake(ipa)         &
                                           + csite%N_leached(ipa) + csite%N_gas_loss(ipa)   &
                                          -    DON_leached                                  !JL

           ! print*, ' csite%mineralized_N_input(ipa)', csite%mineralized_N_input(ipa),     &
           !'structural_N_loss', structural_N_loss,                                         &
           !' fast_N_loss', fast_N_loss, 'slow_C_loss / c2n_slow',slow_C_loss / c2n_slow
           ...
           !------------------------------------------------------------------------------!
            ...
            !I set r_stsc = 1 to make the structural pool decomposition all go to hterotrophic
            ...
            !   write(*,*) 'MSN2', csite%mineralized_soil_N(ipa),'MSN_input'                &
            !,csite%mineralized_N_input(ipa),'MSN_loss',csite%mineralized_N_loss(ipa)!JL!
...
![kg H20/m2/year], this adds up water each day for the entire year
               cgrid%total_evapotranspiration(1)   =  cgrid%total_evapotranspiration(1) +       &
                                         (cpoly%daily_transp(1) * sec_in_day                    &
                                         + cpoly%daily_evap(1) * sec_in_day)
   ...
   !write(*,*) 'MSN2',csite%mineralized_soil_N(ipa),'MSN_input',                           &
   !csite%mineralized_N_input(ipa),'MSN_loss',csite%mineralized_N_loss(ipa)
   return
