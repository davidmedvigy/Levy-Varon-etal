...
module growth_balive
   ...
   subroutine dbalive_dt(cgrid, tfact)
      ...
      !real                          :: nitrogen_supply_test !JL!
      !real                          :: nitrogen_supply_fixer !JL!
      !real                          :: excess_carbon_fixer!JL!
      !real                          :: N_fixation!JL!
      !real			    :: actual_nitrogen_uptake   !XXT
      !real			    :: actual_nitrogen_uptake_fixer   !JL
      ...
      do ipy = 1,cgrid%npolygons
         ...
         do isi = 1,cpoly%nsites
          ...
          !         print*, 'Plants not N limited', ' ipft',ipft, &
          !                 'N_uptake_pot',N_uptake_pot,        &
          !                  ' N supply from soil', cpatch%nitrogen_supply(ico) , &
          !                  'n_plant_lim' ,n_plant_lim,'CB',carbon_balance, &
          !                  'phenology',cpatch%phenology_status(ico)         
            ...
            !             'phenology',  cpatch%phenology_status(ico)
		     ...
		     !soil N uptake
                 ...
                 ! note, I tried using the maximum rate 0.0003941912 and this was way too large
                 ! 1.784159e-05 is the mean observed rate at Sarah's field site(Agua Salud) 
               ! BNF_rate = 1.784159e-05
              !  BNF_rate= 0.0003941912
                BNF_rate= 0
              ...
              !   BNF_rate = 0.0003941912
                 BNF_rate = 0
                  ...
                  !     'fsn', cpatch%fsn(ico),                                            &
                  !     ' cpatch%nitrogen_supply(ico) end',                                &
                  ! cpatch%nitrogen_supply(ico),'CB',carbon_balance,                       &
                  !     'phenology',  cpatch%phenology_status(ico)

                  !if(cpatch%N_fixation(ico)>0)then
                  !   print*,'fixation>0'
                  !   stop
                  ! endif

                  !if(cpatch%shadow_N_uptake(ico)>total_N_supply)then
                  !   print*, 'uptake > supply'
                  !   stop
                  !endif

                  !if(cpatch%actual_nitrogen_uptake(ico)>total_N_supply)then
                  !   print*, 'uptake > supply'
                  !   stop
                  !endif
...
! print*, 'Obligate FIXER START', 'ipft',ipft
! print*, 'shadowN',cpatch%shadow_N_uptake(ico),                                           &
! ' N_uptake_pot', N_uptake_pot ,'CB',carbon_balance,                                      &
! 'phenology',  cpatch%phenology_status(ico)
                  !------------------------------------------------------------------------!                                        
...
! print*, 'N fixation',cpatch%N_fixation(ico)
...
! print*, 'ObligateCost',ObligateCost
...
! print*, 'excess C/ Fixation Cost',cpatch%excess_carbon(ico)
                  !------------------------------------------------------------------------!  
...
!print*, 'cpatch%actual_nitrogen_uptake(ico)', cpatch%actual_nitrogen_uptake(ico)
                     endif
...
!                   print*,'OBLIGATE FIXER END'
                  ...
                  !------------------------------------------------------------------------! 
  ! print*,'GROWTH BALIVE', 'ipa',ipa, 'ico',ico,'ipft',ipft,'cpatch%N_fixation(ico)',cpatch%N_fixation(ico)
...
!print*,'ipa',ipa, 'ico',ico,'ipft',ipft, ' DailyForestFixation',                              & 
!DailyForestFixation,'N_fixation(ico)',cpatch%N_fixation(ico)

                  DailyForestFixation = DailyForestFixation + cpatch%N_fixation(ico) *     &
                                        csite%area(ipa) * cpatch%nplant(ico)
!print*,' DailyForestFixation updated',DailyForestFixation
...
!print*,'DailyForestFixation', DailyForestFixation,         &
!' cgrid%total_N_Fixation(1)', cgrid%total_N_Fixation(1)
                  cgrid%total_N_Fixation(1)  = cgrid%total_N_Fixation(1) + DailyForestFixation !JL!
                  DailyForestFixation= 0.
!print*,'after calculation', ' cgrid%total_N_Fixation(1)',cgrid%total_N_Fixation(1),            &
!'DailyForestFixation', DailyForestFixation
