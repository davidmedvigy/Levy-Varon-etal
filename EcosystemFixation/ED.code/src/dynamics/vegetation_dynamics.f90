...
! print*,'DailyForestFixation', DailyForestFixation, 'DailyFixationDemand', DailyFixationDemand
    ...
    ! the first day of June! JL
     if (current_time%month == 6 .and.current_time%date == 2) then 
        cgrid%annual_evapotranspiration(1) = cgrid%total_evapotranspiration(1)
        cgrid%annual_NPP(1)       = cgrid%total_NPP(1) 
        cgrid%total_N_Fixation(1) = 0.0
        cgrid%total_DON_loss(1)   = 0.0
        cgrid%total_DIN_loss(1)   = 0.0
        cgrid%total_Ngas_loss(1)  = 0.0
        cgrid%total_NPP(1)        = 0.0
        cgrid%total_evapotranspiration(1)  = 0.0
        print*, 'YEARLY ZERO'
     endif
     ...
     do ipy=1,cgrid%npolygons
         ...
         do isi=1,cpoly%nsites
            ...
            enddo
         enddo
!print*, 'VEG before calc', 'ipy',ipy, 'cgrid%total_NPP(1) ',cgrid%total_NPP(1) ,' cgrid%dmean_nppdaily(ipy)/365', cgrid%dmean_nppdaily(ipy)/365
cgrid%total_NPP(1)  = cgrid%total_NPP(1) + cgrid%dmean_nppdaily(ipy)/365 !JL!
!print*, 'VEG after calc', 'cgrid%total_NPP(1) ',cgrid%total_NPP(1) ,' cgrid%dmean_nppdaily(ipy)/365', cgrid%dmean_nppdaily(ipy)/365
