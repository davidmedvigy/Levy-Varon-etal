...
subroutine structural_growth(cgrid, month)
  ...
  use decomp_coms   , only : f_labile                ! ! intent(in)
   ...
   real                          :: old_leaf_hcap
   real                          :: old_wood_hcap
   real                          :: net_seed_N_uptake_tot
   real                          :: net_stem_N_uptake_tot                              !JL
   real                          :: N_storage_pool                                     !JL
   real                          :: nstorage_mort_litter                               !JL
   real                          :: nstorage_in                                        !JL
   real                          :: N_for_seeds                                        !JL
   real                          :: N_wanted_stem                                      !JL
   real                          :: N_wanted_seed                                      !JL
   real                          :: N_wanted_tot                                       !JL
   real                          :: N_have                                             !JL
...
n1=0.;n2=0.;nup=0.;nloss=0.
            ...
            cohortloop: do ico = 1,cpatch%ncohorts
              ...
              !+cpatch%bstorage(ico)/c2n_storage+cpatch%bdead(ico)/c2n_stem(cpatch%pft(ico)))
              ...
              n1=n1+csite%area(ipa)*cpatch%nplant(ico)*(cpatch%balive(ico)/c2n_leaf(ipft)+ &
               ...
               !----- Determine if the plant has enough N to grow/reproduce----------------!
               !----- If it does not then reduce growth/reproduction ----------------------!
               !JL!
               N_wanted_stem = 0.0
               N_wanted_seed = 0.0
               N_wanted_tot  = 0.0
              ...
              !Since this is in a cohort loop and the values are multiplied by nplant,
              !these values are total per cohort per m2 forest
              N_wanted_stem = ((f_bdead * cpatch%bstorage(ico))/c2n_stem(cpatch%pft(ico))) &
                            * cpatch%nplant(ico)
              N_wanted_seed =  (f_bseeds * cpatch%bstorage(ico))/ c2n_recruit(ipft)        &
                            * cpatch%nplant(ico)
              N_wanted_tot =  N_wanted_stem +   N_wanted_seed
              N_have = cpatch%nstorage(ico)*cpatch%nplant(ico)                             &
                     + (cpatch%nitrogen_supply(ico)-cpatch%actual_nitrogen_uptake(ico))    &
                     * cpatch%nplant(ico)

              !print*, 'f_bdead',f_bdead,'f_bseeds',f_bseeds
              if (n_plant_lim .NE. 0 )then
                   if( N_wanted_tot > N_have) then
                     f_bdead = f_bdead *(N_have/ N_wanted_tot)
                     f_bseeds = f_bseeds*(N_have/ N_wanted_tot)
                    endif
              endif

            !Checks
            ! print*,'f_bdead',f_bdead,'f_bseeds',f_bseeds,                               &
            !'N_wanted_tot',N_wanted_tot,'N_wanted_seed', N_wanted_seed,' N_wanted_stem', &
            ! N_wanted_stem,' N_have', N_have
            ! print*, 'plant N uptake STRUCT', csite%total_plant_nitrogen_uptake(ipa),    &
            !'ipa',ipa, 'ico',ico,'MSN',csite%mineralized_soil_N(ipa),'NSTORAGE*nplant',  &
            !cpatch%nstorage(ico)*cpatch%nplant(ico)

            !if(csite%mineralized_soil_N(ipa) <  csite%total_plant_nitrogen_uptake(ipa))then
            !print *, 'PLANT UPTAKE > MSN'
            !stop
            !endif 
               ...
               !net_stem_N_uptake=0.      
               !This is because c2n_stem = c2n_storage
                                                                                        !JL!
               !---------------------------------------------------------------------------!
               !  Determine how much nitrogen is needed for stem growth                    !
               !---------------------------------------------------------------------------!
                                                                                        !JL!
               net_stem_N_uptake = (cpatch%bdead(ico) - bdead_in) * cpatch%nplant(ico)     & 
                                 * ( 1.0 / c2n_stem(cpatch%pft(ico))) 

               !---------------------------------------------------------------------------!
               !  Take the nitrogen from Nstorage before taking it out of the soil         !                                                  !
               !---------------------------------------------------------------------------!

               if (cpatch%nstorage(ico) >=   net_stem_N_uptake/cpatch%nplant(ico) ) then
                  cpatch%nstorage(ico) = cpatch%nstorage(ico)                             &
                                       -  net_stem_N_uptake/cpatch%nplant(ico) 
                  net_stem_N_uptake = 0
               else if (cpatch%nstorage(ico) < net_stem_N_uptake/cpatch%nplant(ico) ) then
                  net_stem_N_uptake = net_stem_N_uptake                                   &
                                    - cpatch%nstorage(ico)*cpatch%nplant(ico) 
                  cpatch%nstorage(ico) = 0
               endif
               ...
               !print*,cpatch%bseeds(ico)/c2n_recruit(ipft)*cpatch%nplant(ico)
               !---------------------------------------------------------------------------!
               !      Rebalance the plant nitrogen uptake considering the actual alloc-    !
               ...
               !---------------------------------------------------------------------------!
               !  Determine how much nitrogen is needed for seed reproduction              !
               ! ation to seeds.  
               !---------------------------------------------------------------------------!
 
                N_for_seeds = cpatch%bseeds(ico) * (1.0 / c2n_recruit(ipft))            !JL

               !---------------------------------------------------------------------------!
               !  Take the nitrogen from Nstorage before taking it out of the soil         !                                                  !
               !---------------------------------------------------------------------------!

               if (cpatch%nstorage(ico) >=  N_for_seeds ) then
                  cpatch%nstorage(ico) = cpatch%nstorage(ico) -  N_for_seeds
                   N_for_seeds = 0
               else if (cpatch%nstorage(ico) <    N_for_seeds ) then
                  N_for_seeds =  N_for_seeds - cpatch%nstorage(ico)
                  cpatch%nstorage(ico) = 0
               endif
               ...
               !---------------------------------------------------------------------------!
                 ...
                 net_seed_N_uptake = N_for_seeds * cpatch%nplant(ico)                  !JL!
               ...
               !n2=n2+csite%area(ipa)*cpatch%nplant(ico)
                 ...
                 !+cpatch%bdead(ico)/c2n_stem(cpatch%pft(ico))                             &
                 !+cpatch%bstorage(ico)/c2n_storage                                        &
                 !+cpatch%bseeds(ico)/c2n_recruit(ipft))
            ...
            nup=nup+csite%area(ipa)*(net_stem_N_uptake+net_seed_N_uptake)
           !print*,'net_seed_N_uptake 2',net_seed_N_uptake,' net_stem_N_uptake',           & 
                 ! net_stem_N_uptake,' total plant n uptake',                               &
                 ! csite%total_plant_nitrogen_uptake(ipa),'NSTORAGE',                      &
                 !cpatch%nstorage(ico)*cpatch%nplant(ico)
      ...
      siteloop: do isi = 1,cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            cohortloop: do ico = 1,cpatch%ncohorts
               ...
               nplant_in   = cpatch%nplant(ico)
               bstorage_in = cpatch%bstorage(ico)
               nstorage_in = cpatch%nstorage(ico)
               agb_in      = cpatch%agb(ico)
               ba_in       = cpatch%basarea(ico)
