...
subroutine normalize_averaged_vars(cgrid,frqsum,dtlsm)
   ...
   do ipy = 1,cgrid%npolygons
      ...
      !  print*, 'csite%avg_rshort_gnd', csite%avg_rshort_gnd,'current time', current_time%hour
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            if (save_daily) then
               ...
               if (save_monthly) then
                 ...
                 csite%mmean_fsn_in(ipa)     = csite%mmean_fsn_in(ipa)                  	   &
                                             + csite%fsn_in(ipa)		     !JL!
		 csite%mmean_total_plant_nitrogen_uptake(ipa) = csite%mmean_total_plant_nitrogen_uptake(ipa)                  	   &
                                             + csite%total_plant_nitrogen_uptake(ipa)	
		 csite%mmean_ssc_in(ipa)     = csite%mmean_ssc_in(ipa)                         &
                                             + csite%ssc_in(ipa)		     !JL!
               end if
            end if
            ...
            cohortloop: do ico=1,cpatch%ncohorts
               ...
               if (save_monthly) then 
                  cpatch%mmean_gpp(ico)           = cpatch%mmean_gpp(ico)                  &
                                                  + cpatch%dmean_gpp(ico)                           ! JL do not have a monthly mean varible so skippint this part
...
!JL!
!cpatch%mmean_nitrogen_uptake(ico)    = cpatch%mmean_nitrogen_uptake(ico)           &
 !                                                 + cpatch%nitrogen_uptake(ico)
!JL!
!cpatch%mmean_N_uptake_pot(ico)    = cpatch%mmean_N_uptake_pot(ico)           &
...
!JL!
 !cgrid%mmean_nitrogen_uptake(ipy)= cgrid%mmean_nitrogen_uptake(ipy)                          &
 !                                   + cgrid%nitrogen_uptake(ipy)
!JL!
 !cgrid%mmean_N_uptake_pot(ipy)= cgrid%mmean_N_uptake_pot(ipy)                          &
  !                                  + cgrid%dmean_N_uptake_pot(ipy)
!JL!
...
subroutine zero_ed_daily_output_vars(cgrid)
   ...
   do ipy = 1,cgrid%npolygons
      ...
      cpoly%daily_pcpg(1)        = cgrid%dmean_pcpg(ipy)       !JL!
      cpoly%daily_evap(1)        = cgrid%dmean_evap(ipy)       !JL!
      cpoly%daily_transp(1)      = cgrid%dmean_transp(ipy)     !JL!

   !   print*,'AVG UTILS', 'ipy',ipy,'daily_pcpg',cpoly%daily_pcpg(1), &
   !   'evap',cpoly%daily_evap(1),'transp', cpoly%daily_transp(1)
...
subroutine normalize_ed_monthly_output_vars(cgrid)
 ...
 !cgrid%mmean_nitrogen_uptake     (ipy) = cgrid%mmean_nitrogen_uptake    (ipy) * ndaysi !JL!
 !cgrid%mmean_N_uptake_pot     (ipy) = cgrid%mmean_N_uptake_pot     (ipy) * ndaysi !JL!
      ...
      siteloop: do isi = 1, cpoly%nsites
         ...
         patchloop: do ipa=1,csite%npatches
            ...
            csite%mmean_fsn_in(ipa)          = csite%mmean_fsn_in(ipa)          * ndaysi !JL!
	    csite%mmean_ssc_in(ipa)          = csite%mmean_ssc_in(ipa)          * ndaysi !JL!
            csite%mmean_total_plant_nitrogen_uptake(ipa)  = csite%mmean_total_plant_nitrogen_uptake(ipa)  * ndaysi !JL!           
...
! cpatch%mmean_nitrogen_uptake  (ico) = cpatch%mmean_nitrogen_uptake  (ico) * ndaysi!JL!
! cpatch%mmean_N_uptake_pot  (ico) = cpatch%mmean_N_uptake_pot  (ico) * ndaysi !JL!
...
!cgrid%mmean_N_uptake_pot         (ipy) = 0. !JL!
!cgrid%mmean_nitrogen_uptake         (ipy) = 0. !JL!
      ...
      do isi = 1, cpoly%nsites
         ...
         do ipa=1,csite%npatches
            ...
            csite%mmean_fsn_in            (ipa) = 0. !JL!
	    csite%mmean_ssc_in            (ipa) = 0. !JL!
	    csite%mmean_total_plant_nitrogen_uptake          (ipa) = 0. !JL!            
 ...
 !cpatch%mmean_nitrogen_uptake       (ico) = 0. !JL!
 !cpatch%mmean_N_uptake_pot        (ico) = 0. !JL!
...
!==========================================================================================!
!==========================================================================================!
!    This subroutine integrates the daily average precipitation that will be used for Nitrogen  !   
! leaching and gas loss calculations. This subroutine allows the N losses to be calculated                                        !
! even if output files are not written.  Added by Jen Levy                                                                                             !
!------------------------------------------------------------------------------------------!
subroutine integrate_precipitation(cgrid)
   use ed_state_vars        , only : edtype              & ! structure
                                   , polygontype         & ! structure
                                   , sitetype            & ! structure
                                   , patchtype           ! ! structure
...
print*,  '  cgrid%dmean_pcpg      (ipy) ', cgrid%dmean_pcpg  (ipy), 'cgrid%avg_pcpg(ipy)', cgrid%avg_pcpg (ipy)
   return
end subroutine integrate_precipitation
!==========================================================================================!
!==========================================================================================!
!JL!
!==========================================================================================!
!==========================================================================================!
subroutine normalize_precipitation(cgrid)
   use ed_state_vars        , only : edtype        & ! structure
                                   , polygontype   & ! structure
                                   , sitetype      & ! structure
                                   , patchtype     ! ! structure
 ...
 !----- Compute the normalization factors. This is done once. ---------------------------!
   ...
   return
 end subroutine normalize_precipitation
!==========================================================================================!
!==========================================================================================!
...
! to compute the monthly mean (in case the latter was requested).                          !
!------------------------------------------------------------------------------------------!
subroutine zero_precipitation(cgrid)
   use ed_state_vars, only : edtype        & ! structure
                           , polygontype   & ! structure
                           , sitetype      & ! structure
                           , patchtype     ! ! structure
   ...
   do ipy = 1,cgrid%npolygons
     ...
     !print*,'ipy',ipy,'daily_pcpg',cpoly%daily_pcpg(1)
     ! cpoly%ForestN(1)           = cgrid%ForestN(ipy)          !JL! 
     ! cpoly%SoilN(1)             = cgrid%SoilN(ipy)            !JL!
     ! cpoly%PlantN(1)            = cgrid%PlantN(ipy)           !JL!
     ! cpoly%total_N_Fixation(1)  = cgrid%total_N_Fixation(ipy) !JL!
    !  cpoly%total_DON_loss(1)    = cgrid%total_DON_loss(ipy)   !JL!
    !  cpoly%total_DIN_loss(1)    = cgrid%total_DIN_loss(ipy)   !JL!
    !  cpoly%total_Ngas_loss(1)   = cgrid%total_Ngas_loss(ipy)  !JL!
   ...
   return
end subroutine zero_precipitation
!==========================================================================================!
!==========================================================================================!
