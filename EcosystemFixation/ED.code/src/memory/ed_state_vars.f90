...
!********************************************************
     !Added by JL,XXT, excess_carbon due to nitrogen limitation
     real ,pointer,dimension(:) :: excess_carbon
     real ,pointer,dimension(:) :: excess_carbon_fixer 
real ,pointer,dimension(:) :: nitrogen_supply_fixer
real ,pointer,dimension(:) :: nitrogen_supply
real ,pointer,dimension(:) :: shadow_N_uptake
real ,pointer,dimension(:) :: actual_nitrogen_uptake 
real ,pointer,dimension(:) :: actual_nitrogen_uptake_fixer
real ,pointer,dimension(:) :: fixation_demand 
real ,pointer,dimension(:) :: N_fixation 
real ,pointer,dimension(:) :: fsn_fixer
...
!JL!
      !Plant storage pool of nitroge [kgN/plant]
     real ,pointer,dimension(:) :: nstorage
!JL!
 ...
 !    real, pointer,dimension(:,:) :: N_limitation_factor ! JL, can change running average to any number cb, cbr_bar
    ...
    ! real, pointer,dimension(:) :: N_limitation_factor_bar !JL!
    ...
    ! real, pointer, dimension(:)   :: mmean_nitrogen_supply  !JL!
     !real, pointer, dimension(:)   :: mmean_N_uptake_pot  !JL!
 ...
 real ,pointer,dimension(:) :: carbon_balance !JL!
 real ,pointer,dimension(:) :: N_uptake_pot !JL!
 real ,pointer,dimension(:) :: carbon_balance_pot !JL!
 real ,pointer,dimension(:) :: nitrogen_uptake !JL!
 real ,pointer,dimension(:) :: nitrogen_supply_test !JL!
 real ,pointer,dimension(:) :: nitrogen_supply_test2 !JL!
...
!============================================================================!
  ...
  type sitetype
     ...
     real , pointer,dimension(:) :: N_leached  !JL!
     real , pointer,dimension(:) :: N_gas_loss  !JL!
     ...
     real , pointer,dimension(:) :: dist_fast_soil_N !JL!
     real , pointer,dimension(:) :: dist_structural_soil_C !JL!
     real , pointer,dimension(:) :: mmean_fsn_in   !JL!
     real , pointer,dimension(:) :: mmean_ssc_in  !JL!
     real , pointer,dimension(:) :: mmean_total_plant_nitrogen_uptake  !JL!
  ...
  real , pointer,dimension(:) :: fsc_in_no_excess !JL!
     ...
     ! Dissolved organic nitrogen to be lost when leaching occurs [kgN/m2] !JL!
     real , pointer,dimension(:) :: interval_DON_loss
...
!============================================================================!
  ...
  type polygontype
     ...
     !----------------------------------------------------------------!
     !JL N cycle variables
     real, pointer,dimension(:)  :: daily_pcpg            ! [kg_H2O/m2/s]
     real, pointer,dimension(:)  :: daily_transp          ! [kg_H2O/m2/s]
     real, pointer,dimension(:)  :: daily_evap            ! [kg_H2O/m2/s]
     real, pointer,dimension(:)  :: PlantN           
     real, pointer,dimension(:)  :: ForestN         
     real, pointer,dimension(:)  :: SoilN          

     ! These variables are set to 0 once a year (at the end of May)
     ! after the annual output is recorded( vegetation_dynamics.f90)
     real, pointer,dimension(:)  :: total_N_Fixation         
     real, pointer,dimension(:)  :: total_DON_loss         
     real, pointer,dimension(:)  :: total_DIN_loss       
     real, pointer,dimension(:)  :: total_Ngas_loss   
     real, pointer,dimension(:)  :: total_NPP
     real, pointer,dimension(:)  :: annual_NPP
     real, pointer,dimension(:)  :: annual_evapotranspiration       
     real, pointer,dimension(:)  :: total_evapotranspiration   
...
!============================================================================!
  ...
  type edtype
     ...
     real,pointer,dimension(:) :: avg_fsn
     real,pointer,dimension(:) :: avg_msn
     real,pointer,dimension(:) :: daily_pcpg               !JL
     real,pointer,dimension(:) :: daily_evap               !JL
     real,pointer,dimension(:) :: daily_transp             !JL
     real,pointer,dimension(:) :: PlantN                   !JL
     real,pointer,dimension(:) :: ForestN                  !JL
     real,pointer,dimension(:) :: SoilN                    !JL
     real,pointer,dimension(:) :: total_N_Fixation         !JL
     real,pointer,dimension(:) :: total_DON_loss           !JL
     real,pointer,dimension(:) :: total_DIN_loss           !JL
     real,pointer,dimension(:) :: total_Ngas_loss          !JL
     real,pointer,dimension(:) :: total_NPP                !JL
     real,pointer,dimension(:) :: total_evapotranspiration !JL
     real,pointer,dimension(:) :: annual_NPP               !JL
     real,pointer,dimension(:) :: annual_evapotranspiration !JL
     ...
     !real, pointer, dimension(:)   :: mmean_nitrogen_supply     ! [ kgC/m²/yr]!JL!
     !real, pointer, dimension(:)   :: mmean_N_uptake_pot     ! [ kgC/m²/yr]!JL!
...
!! NOT USED       allocate(cgrid%tau(npolygons))
       ...
       allocate(cgrid%daily_pcpg       (npolygons))!JL
       allocate(cgrid%daily_evap       (npolygons))!JL
       allocate(cgrid%daily_transp     (npolygons))!JL
       allocate(cgrid%PlantN           (npolygons))!JL
       allocate(cgrid%ForestN          (npolygons))!JL
       allocate(cgrid%SoilN            (npolygons))!JL
       allocate(cgrid%total_N_Fixation (npolygons))!JL
       allocate(cgrid%total_DON_loss   (npolygons))!JL
       allocate(cgrid%total_DIN_loss   (npolygons))!JL
       allocate(cgrid%total_Ngas_loss  (npolygons))!JL
       allocate(cgrid%total_DON_loss   (npolygons))!JL
       allocate(cgrid%total_NPP        (npolygons))!JL
       allocate(cgrid%total_evapotranspiration (npolygons))!JL
       allocate(cgrid%annual_NPP       (npolygons))!JL
       allocate(cgrid%annual_evapotranspiration(npolygons))!JL
...
! allocate(cgrid%mmean_nitrogen_supply     (             npolygons))!JL!
 !allocate(cgrid%mmean_N_uptake_pot     (             npolygons)) !JL!
...
!============================================================================!
  subroutine allocate_polygontype(cpoly,nsites)
    ...
    allocate(cpoly%PlantN                  (nsites))!JL!
    allocate(cpoly%ForestN                 (nsites))!JL!
    allocate(cpoly%SoilN                   (nsites))!JL!
    allocate(cpoly%total_N_Fixation        (nsites))!JL!
    allocate(cpoly%total_DON_loss          (nsites))!JL!
    allocate(cpoly%total_DIN_loss          (nsites))!JL!
    allocate(cpoly%total_Ngas_loss         (nsites))!JL!
    allocate(cpoly%daily_pcpg              (nsites))!JL!
    allocate(cpoly%daily_evap              (nsites))!JL!
    allocate(cpoly%daily_transp            (nsites))!JL!
    allocate(cpoly%total_NPP               (nsites))!JL!
    allocate(cpoly%total_evapotranspiration (nsites))!JL!
    allocate(cpoly%annual_NPP               (nsites))!JL!
    allocate(cpoly%annual_evapotranspiration (nsites))!JL!
...
!============================================================================!
  subroutine allocate_sitetype(csite,npatches)
    ...
    allocate(csite%N_leached(npatches))!JL!
    allocate(csite%N_gas_loss(npatches))!JL!
    ...
    allocate(csite%fsc_in_no_excess (npatches))        !JL
    allocate(csite%dist_fast_soil_N (npatches))        !JL    
    allocate(csite%dist_structural_soil_C (npatches))  !JL
    allocate(csite%interval_DON_loss (npatches))       !JL
    ...
    if (imoutput > 0 .or. iqoutput > 0) then
       ...
       allocate(csite%mmean_fsn_in            (npatches)) !JL!
       allocate(csite%mmean_ssc_in            (npatches)) !JL!
       allocate(csite%mmean_total_plant_nitrogen_uptake       (npatches)) !JL!
...
!********************************************************
     !Added by JL,XXT, excess_carbon due to nitrogen limitation
allocate(cpatch%excess_carbon(ncohorts))
allocate(cpatch%excess_carbon_fixer(ncohorts))
allocate(cpatch%nitrogen_supply_fixer(ncohorts))
allocate(cpatch%nitrogen_supply(ncohorts))
allocate(cpatch%shadow_N_uptake(ncohorts))
allocate(cpatch%actual_nitrogen_uptake (ncohorts))
allocate(cpatch%actual_nitrogen_uptake_fixer(ncohorts))
allocate(cpatch%fixation_demand (ncohorts))
allocate(cpatch%fsn_fixer (ncohorts))
allocate(cpatch%N_fixation(ncohorts))
    ...
    allocate(cpatch%nstorage(ncohorts))!JL!
    allocate(cpatch%cb(13,ncohorts))
   ! allocate(cpatch%N_limitation_factor (FSN_ndays_to_avg,ncohorts)) ! JL this is where  FSN_ndays_to_avg will change the number of days averaged
    allocate(cpatch%cb_max(13,ncohorts))
    allocate(cpatch%cbr_bar(ncohorts))
   ! allocate(cpatch%N_limitation_factor_bar(ncohorts)) !JL!
    ...
    allocate(cpatch%carbon_balance(ncohorts)) !JL!
    allocate(cpatch%N_uptake_pot(ncohorts)) !JL!
    allocate(cpatch%nitrogen_uptake(ncohorts)) !JL!
allocate(cpatch%nitrogen_supply_test(ncohorts)) !JL!
allocate(cpatch%nitrogen_supply_test2(ncohorts)) !JL!
    allocate(cpatch%carbon_balance_pot(ncohorts)) !JL!
    ...
    !***********************************************
    !Added by XXT, excess_carbon due to nitrogen limitation
  !  allocate(cpatch%excess_carbon(ncohorts)) 
    !***********************************************
...
! allocate(cpatch%mmean_nitrogen_supply(ncohorts))!JL!
 !allocate(cpatch%mmean_N_uptake_pot(ncohorts))!JL!
...
!============================================================================!
  subroutine nullify_edtype(cgrid)
    ...
    type(edtype),target :: cgrid
       ...
       nullify(cgrid%daily_pcpg        ) !JL
       nullify(cgrid%daily_evap        ) !JL
       nullify(cgrid%daily_transp      ) !JL
       nullify(cgrid%PlantN            ) !JL
       nullify(cgrid%ForestN           ) !JL
       nullify(cgrid%SoilN             ) !JL
       nullify(cgrid%total_N_Fixation  ) !JL
       nullify(cgrid%total_DON_loss    ) !JL
       nullify(cgrid%total_DIN_loss    ) !JL
       nullify(cgrid%total_Ngas_loss   ) !JL
       nullify(cgrid%total_NPP         ) !JL
       nullify(cgrid%total_evapotranspiration   ) !JL
       nullify(cgrid%annual_NPP        ) !JL
       nullify(cgrid%annual_evapotranspiration   ) !JL
 ...
 !nullify(cgrid%mmean_nitrogen_supply        )!JL!
 !nullify(cgrid%mmean_N_uptake_pot        )!JL!
...
!============================================================================!
  subroutine nullify_polygontype(cpoly)
    ...
    nullify(cpoly%daily_pcpg       )!JL!
    nullify(cpoly%daily_evap       )!JL!
    nullify(cpoly%daily_transp     )!JL!
    nullify(cpoly%PlantN           )!JL!
    nullify(cpoly%ForestN          )!JL!
    nullify(cpoly%SoilN            )!JL!
    nullify(cpoly%total_N_Fixation )!JL!
    nullify(cpoly%total_DON_loss   )!JL!
    nullify(cpoly%total_DIN_loss   )!JL!
    nullify(cpoly%total_Ngas_loss  )!JL!
    nullify(cpoly%total_NPP        )!JL!
    nullify(cpoly%total_evapotranspiration  )!JL!
    nullify(cpoly%annual_NPP        )!JL!
    nullify(cpoly%annual_evapotranspiration  )!JL!
...
!============================================================================!
  subroutine nullify_sitetype(csite)
    ...
    nullify(csite%N_leached)!JL!
    nullify(csite%N_gas_loss)!JL!
    ...
    nullify(csite%mmean_fsn_in)                            !JL
    nullify(csite%mmean_ssc_in)                            !JL
    nullify(csite%mmean_total_plant_nitrogen_uptake)       !JL
    nullify(csite%dist_fast_soil_N)                        !JL
    nullify(csite%dist_structural_soil_C)                  !JL
    nullify(csite%fsc_in_no_excess)                        !JL
    nullify(csite%interval_DON_loss)                       !JL
    nullify(csite%repro)
...
!*********************JL,XXT
 nullify(cpatch%excess_carbon)
 nullify(cpatch%excess_carbon_fixer)
 nullify(cpatch%nitrogen_supply_fixer)
 nullify(cpatch%nitrogen_supply)
 nullify(cpatch%shadow_N_uptake)
 nullify(cpatch%actual_nitrogen_uptake)
 nullify(cpatch%actual_nitrogen_uptake_fixer)
 nullify(cpatch%fixation_demand)
 nullify(cpatch%N_fixation)
 nullify(cpatch%fsn_fixer)
...
!***************************
    ...
    nullify(cpatch%nstorage)!JL!
    nullify(cpatch%cb)
!nullify(cpatch%N_limitation_factor) !JL!
!nullify(cpatch%N_limitation_factor_bar) !JL!
   ...
   ! nullify(cpatch%mmean_nitrogen_supply        )!JL!
   ! nullify(cpatch%mmean_N_uptake_pot        )!JL!
    ...
    nullify(cpatch%fsn)
 nullify(cpatch%carbon_balance) !JL!
 nullify(cpatch%N_uptake_pot)!JL!
nullify(cpatch%nitrogen_uptake)!JL!
nullify(cpatch%nitrogen_supply_test)!JL!
nullify(cpatch%nitrogen_supply_test2)!JL!
nullify(cpatch%carbon_balance_pot)!JL!
...
!============================================================================!
  ...
  ! ===================================================
    ...
    type(edtype),target :: cgrid
       ...
       if(associated(cgrid%daily_pcpg              )) deallocate(cgrid%daily_pcpg          )!JL
       if(associated(cgrid%daily_evap              )) deallocate(cgrid%daily_evap          )!JL
       if(associated(cgrid%daily_transp            )) deallocate(cgrid%daily_transp        )!JL
       if(associated(cgrid%PlantN                  )) deallocate(cgrid%PlantN              )!JL
       if(associated(cgrid%ForestN                 )) deallocate(cgrid%ForestN             )!JL     
       if(associated(cgrid%SoilN                   )) deallocate(cgrid%SoilN               )!JL
       if(associated(cgrid%total_N_Fixation        )) deallocate(cgrid%total_N_Fixation    )!JL
       if(associated(cgrid%total_DON_loss          )) deallocate(cgrid%total_DON_loss      )!JL
       if(associated(cgrid%total_DIN_loss          )) deallocate(cgrid%total_DIN_loss      )!JL
       if(associated(cgrid%total_Ngas_loss         )) deallocate(cgrid%total_Ngas_loss     )!JL
       if(associated(cgrid%total_NPP               )) deallocate(cgrid%total_NPP           )!JL
       if(associated(cgrid%total_evapotranspiration)) deallocate(cgrid%total_evapotranspiration)!JL
       if(associated(cgrid%annual_NPP              )) deallocate(cgrid%annual_NPP          )!JL
       if(associated(cgrid%annual_evapotranspiration)) deallocate(cgrid%annual_evapotranspiration)!JL
 ...
 !if(associated(cgrid%mmean_nitrogen_suppply        )) deallocate(cgrid%mmean_nitrogen_supply        )!JL!
 !if(associated(cgrid%mmean_N_uptake_pot        )) deallocate(cgrid%mmean_N_uptake_pot        )!JL!
...
!============================================================================!
  subroutine deallocate_polygontype(cpoly)
    ...
    if(associated(cpoly%PlantN                    )) deallocate(cpoly%PlantN                    )!JL
    ...
    if(associated(cpoly%total_NPP                 )) deallocate(cpoly%total_NPP                 )!JL
    if(associated(cpoly%total_evapotranspiration  )) deallocate(cpoly%total_evapotranspiration  )!JL
    if(associated(cpoly%annual_NPP                 )) deallocate(cpoly%annual_NPP                 )!JL
    if(associated(cpoly%annual_evapotranspiration  )) deallocate(cpoly%annual_evapotranspiration  )!JL
...
!============================================================================!
  subroutine deallocate_sitetype(csite)
    ...
    if(associated(csite%dist_fast_soil_N             )) deallocate(csite%dist_fast_soil_N             )        !JL
    if(associated(csite%dist_structural_soil_C       )) deallocate(csite%dist_structural_soil_C       )        !JL
...
!********************************************************************************   
    ...
    if(associated(cpatch%nstorage))            deallocate(cpatch%nstorage)!JL!
...
! if(associated(cpatch%mmean_nitrogen_supply ))   deallocate(cpatch%mmean_nitrogen_supply  )!JL!
! if(associated(cpatch%mmean_N_uptake_pot ))   deallocate(cpatch%mmean_N_uptake_pot  )!JL!
    ...
    if(associated(cpatch%carbon_balance))      deallocate(cpatch%carbon_balance)!JL!
    if(associated(cpatch%N_uptake_pot))           deallocate(cpatch%N_uptake_pot) !JL!
    if(associated(cpatch%nitrogen_uptake))      deallocate(cpatch%nitrogen_uptake)!JL!
    if(associated(cpatch%carbon_balance_pot))      deallocate(cpatch%carbon_balance_pot)!JL!
 if(associated(cpatch%nitrogen_supply_test))      deallocate(cpatch%nitrogen_supply_test)!JL!
 if(associated(cpatch%nitrogen_supply_test2))      deallocate(cpatch%nitrogen_supply_test2)!JL!
    ...
    !*************************************************************************************
    !Added by XXT, excess_carbon due to nitrogen limitaiton
    !if(associated(cpatch%excess_carbon))       deallocate(cpatch%excess_carbon)
    !*************************************************************************************
...
!============================================================================!
   ...
   subroutine copy_sitetype(isite,osite,ipaa,ipaz,opaa,opaz)
      ...
      opaloop: do opa = opaa, opaz
         ...
         osite%dist_fast_soil_N(opa)            = isite%dist_fast_soil_N(ipa)                !JL
	 osite%dist_structural_soil_C(opa)      = isite%dist_structural_soil_C(ipa)          !JL
...
!============================================================================!
  subroutine copy_sitetype_mask(sitein,siteout,logmask,masksz,newsz)
    ...
    siteout%dist_fast_soil_N(1:inc)     = pack(sitein%dist_fast_soil_N,logmask)          !JL
    siteout%dist_structural_soil_C(1:inc) = pack(sitein%dist_structural_soil_C,logmask)  !JL
...
!************************************************************************   
 patchout%lai(1:inc)              = pack(patchin%lai,mask)
    ...
    patchout%nstorage(1:inc)         = pack(patchin%nstorage,mask)!JL!
    patchout%cbr_bar(1:inc)          = pack(patchin%cbr_bar,mask)
!patchout%N_limitation_factor_bar(1:inc)          = pack(patchin%N_limitation_factor_bar,mask) !JL
    ...
    patchout%carbon_balance(1:inc)   = pack(patchin%carbon_balance,mask) !JL!
    patchout%N_uptake_pot(1:inc)     = pack(patchin%N_uptake_pot,mask) !JL!
    patchout%nitrogen_uptake(1:inc)     = pack(patchin%nitrogen_uptake,mask) !JL!
    patchout%carbon_balance_pot(1:inc)     = pack(patchin%carbon_balance_pot,mask) !JL!
patchout%nitrogen_supply_test(1:inc)     = pack(patchin%nitrogen_supply_test,mask) !JL!
patchout%nitrogen_supply_test2(1:inc)     = pack(patchin%nitrogen_supply_test2,mask) !JL!
   ...
   ! patchout%excess_carbon(1:inc)    = pack(patchin%excess_carbon,mask)
    ...
    do m=1,inc
      ...
      !patchout%N_limitation_factor(i,m)               = patchin%N_limitation_factor(i,k) !JL
      ! end do
...
!********************************************************************      
 patchout%lai(iout)              = patchin%lai(iin)
       ...
       patchout%nstorage(iout)         = patchin%nstorage(iin)!JL!
       patchout%cb(:,iout)             = patchin%cb(:,iin)
    !   patchout%N_limitation_factor(:,iout)             = patchin%N_limitation_factor(:,iin) !JL
  !     patchout%N_limitation_factor_bar(iout)             = patchin%N_limitation_factor_bar(iin) !JL
       ...
       patchout%carbon_balance(iout)    = patchin%carbon_balance(iin) !JL!
       patchout%N_uptake_pot(iout)      = patchin%N_uptake_pot(iin) !JL!
       patchout%nitrogen_uptake(iout)      = patchin%nitrogen_uptake(iin) !JL!
       patchout%carbon_balance_pot(iout)      = patchin%carbon_balance_pot(iin) !JL!
       patchout%nitrogen_supply_test(iout)      = patchin%nitrogen_supply_test(iin) !JL!
       patchout%nitrogen_supply_test2(iout)      = patchin%nitrogen_supply_test2(iin) !JL!
   ...
   subroutine filltab_edtype_p11(cgrid,igr,init,var_len,var_len_global,max_ptrs,nvar)
      ...
      if (associated(cgrid%avg_bstorage)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_bstorage,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_BSTORAGE :11:hist:dail:anal:year') 
      ...
      if (associated(cgrid%avg_bseeds)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_bseeds,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_BSEEDS :11:hist:dail:anal:year') 
      ...
      if (associated(cgrid%avg_balive)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_balive,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_BALIVE :11:hist:dail:anal:year') 
      ...
      if (associated(cgrid%avg_bleaf)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_bleaf,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_BLEAF :11:hist:dail:anal:year') 
      ...
      if (associated(cgrid%avg_broot)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_broot,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_BROOT :11:hist:dail:anal:year') 
      ...
      if (associated(cgrid%avg_bsapwood)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_bsapwood,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_BSAPWOOD :11:hist:dail:anal:year') 
      ...
      if (associated(cgrid%avg_fsc)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_fsc,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_FSC :11:hist:anal:opti:dail:mont:year') 
      ...
      if (associated(cgrid%avg_ssc)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_ssc,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_SSC :11:hist:anal:opti:dail:mont:year') 
      ...
      if (associated(cgrid%avg_stsc)) then
         ...
         call vtable_edio_r(npts,cgrid%avg_stsc,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'AVG_STSC :11:hist:anal:opti:year:mont:dail') 
    ...
    !_________JL ADDITIONS_________________________________________________________________!
!JL!
      if (associated(cgrid%daily_pcpg)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%daily_pcpg,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'DAILY_PCPG :11:hist:anal:opti:dail:year:mont') 
         call metadata_edio(nvar,igr,'Poly Avg. Daily precipitation rate','[kg H20/m2/sec]','ipoly') 
      end if

      if (associated(cgrid%daily_evap)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%daily_evap,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'DAILY_EVAP :11:hist:anal:opti:dail:year:mont') 
         call metadata_edio(nvar,igr,'Poly Avg. Daily evaporation rate','[kg H20/m2/sec]','ipoly') 
      end if
      if (associated(cgrid%daily_transp)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%daily_transp,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'DAILY_TRANSP :11:hist:anal:opti:dail:year:mont') 
         call metadata_edio(nvar,igr,'Poly Avg. Daily transpiration rate','[kg H20/m2/sec]','ipoly') 
      end if

      if (associated(cgrid%PlantN)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%PlantN,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'PlantN :11:hist:anal:opti:dail:year:mont') 
         call metadata_edio(nvar,igr,'Poly PlantN','[kg N/m2 forest]','ipoly') 
      end if

      if (associated(cgrid%SoilN)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%SoilN,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'SOILN :11:hist:anal:opti:mont:dail:year') 
      ...
      if (associated(cgrid%ForestN)) then
         ...
         call vtable_edio_r(npts,cgrid%ForestN,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'ForestN :11:hist:anal:opti:dail:year:mont') 
      ...
      if (associated(cgrid%total_N_Fixation)) then
         ...
         call vtable_edio_r(npts,cgrid%total_N_Fixation,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'total_N_Fixation :11:hist:anal:opti:mont:dail:year') 
      ...
      if (associated(cgrid%total_DON_loss)) then
         ...
         call vtable_edio_r(npts,cgrid%total_DON_loss,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'total_DON_loss:11:hist:anal:opti:mont:dail:year') 
      ...
      if (associated(cgrid%total_NPP)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%total_NPP,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'total_NPP :11:hist:anal:opti:dail:year:mont') 
         call metadata_edio(nvar,igr,'total_NPP','[kg C/m2 forest/yr]','ipoly') 
      end if

      if (associated(cgrid%total_evapotranspiration)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%total_Ngas_loss,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'total_evapotranspiration :11:hist:anal:opti:dail:year:mont') 
         call metadata_edio(nvar,igr,'total_evapotranspiration','[kg h20/m2 forest/day]','ipoly') 
      end if

      if (associated(cgrid%annual_NPP)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%annual_NPP,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'annual_NPP :11:hist:anal:opti:dail:year:mont') 
         call metadata_edio(nvar,igr,'annual_NPP','[kg N/m2 forest]','ipoly') 
      end if

      if (associated(cgrid%annual_evapotranspiration)) then
         nvar=nvar+1
         call vtable_edio_r(npts,cgrid%annual_evapotranspiration,nvar,igr,init,cgrid%pyglob_id, &
              var_len,var_len_global,max_ptrs,'annual_evapotranspiration :11:hist:anal:opti:dail:year:mont') 
         call metadata_edio(nvar,igr,'annual_evapotranspiration','[kg N/m2 forest]','ipoly') 
      end if

        !_________JL ADDITIONS END________________________________________________________!
   ...
   subroutine filltab_polygontype(igr,ipy,init)
      ...
      if (associated(cpoly%basal_area_growth)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'BASAL_AREA_GROWTH :246:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpoly%agb_growth)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'AGB_GROWTH :246:hist:mont:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(cpoly%basal_area_mort)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'BASAL_AREA_MORT :246:hist:year:mont') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
   ...
   subroutine filltab_sitetype(igr,ipy,isi,init)
      ...
      if (associated(csite%mineralized_soil_N)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'MINERALIZED_SOIL_N :31:hist:dail:year') !JL! added dail
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%dist_structural_soil_C)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%dist_structural_soil_C,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'DIST_STRUCTURAL_SOIL_C :31:hist:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      if (associated(csite%fsc_in)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'FSC_IN :31:dail:hist:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
     if (associated(csite%ssc_in)) then
      ...
      if (associated(csite%total_plant_nitrogen_uptake)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'TOTAL_PLANT_NITROGEN_UPTAKE :31:dail:hist') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
      ...
      !_________!JL!___________________________________________________!
!JL
     ...
     if (associated(csite%N_leached)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'N_LEACHED :31:hist:dail:year') !JL! added dail
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
     ...
     if (associated(csite%N_gas_loss)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'N_GAS_LOSS :31:hist:dail:year') !JL! added dail
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if

      if (associated(csite%dist_fast_soil_N)) then
         nvar=nvar+1
           call vtable_edio_r(npts,csite%dist_fast_soil_N,nvar,igr,init,csite%paglob_id, &
           var_len,var_len_global,max_ptrs,'DIST_FAST_SOIL_N :31:hist:year') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
   ...
   subroutine filltab_patchtype(igr,ipy,isi,ipa,init)
      ...
      use ed_var_tables, only : vtable_edio_r & ! sub-routine
                              , vtable_edio_i & ! sub-routine
                              , metadata_edio ! ! sub-routine
     ! use decomp_coms,   only : FSN_ndays_to_avg
      implicit none
      !----- Arguments. -------------------------------------------------------------------!
      integer          , intent(in) :: init
      integer          , intent(in) :: igr
...
!*******************************************************************************************JL,XXT
 ...
 if (associated(cpatch%fixation_demand)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'FIXATION_DEMAND :41:hist:year:dail:anal:mont:dcyc') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
...
!JL!
 !if (associated(cpatch%N_limitation_factor_bar)) then
  ...
  !         var_len,var_len_global,max_ptrs,'N_LIMITATION_FACTOR_BAR :41:hist:dail:mont:year:dcyc') 
  !       call metadata_edio(nvar,igr,'Annual average ratio of cb/cb_max','[NA]','NA') 
  !    end if
!JL!
...
! if (associated(cpatch%mmean_nitrogen_supply)) then
 !        nvar=nvar+1
  !       call vtable_edio_r(npts,cpatch%mmean_nitrogen_supply,nvar,igr,init,cpatch%coglob_id, &
...
!JL!
      ...
      if (associated(cpatch%fsn)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'FSN :41:hist:dail:mont:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
!JL!
 if (associated(cpatch%carbon_balance)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'CARBON_BALANCE :41:hist:dail:mont:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
!JL!
!JL!
 if (associated(cpatch%N_uptake_pot)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'N_UPTAKE_POT :41:hist:dail:mont:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
!JL!
!JL!
 if (associated(cpatch%nitrogen_uptake)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'NITROGEN_UPTAKE :41:hist:dail:mont:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
!JL!
!JL!
 if (associated(cpatch%carbon_balance_pot)) then
         nvar=nvar+1
           ...
           var_len,var_len_global,max_ptrs,'CARBON_BALANCE_POT:41:hist:dail:mont:anal') 
         call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      end if
!JL!
!JL!
...
!JL!
     ...
     ! if (associated(cpatch%excess_carbon)) then
      ...
      !if (associated(cpatch%excess_carbon_fixer)) then
       !  nvar=nvar+1
        !   call vtable_edio_r(npts,cpatch%excess_carbon_fixer,nvar,igr,init,cpatch%coglob_id, &
         !  var_len,var_len_global,max_ptrs,'EXCESS_CARBON_fixer :41:hist:dail') 
        ! call metadata_edio(nvar,igr,'No metadata available','[NA]','NA') 
      !end if
      !***************************************************************************************
...
!JL!
 !if (associated(cpatch%N_limitation_factor)) then
  !       nvar=nvar+1
   !        call vtable_edio_r(cpatch%ncohorts * FSN_ndays_to_avg,cpatch%N_limitation_factor,nvar,igr,init,cpatch%coglob_id, &
    !       var_len,var_len_global,max_ptrs,'N_LIMITATION_FACTOR :49:hist:dail:mont:dcyc:year') 
     !    call metadata_edio(nvar,igr,'carbon balance previous 12 months+current','[kgC/plant]','13 - icohort') 
      !end if
!JL!
