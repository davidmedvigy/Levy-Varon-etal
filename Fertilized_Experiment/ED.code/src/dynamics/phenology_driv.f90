...
subroutine update_phenology(doy, cpoly, isi, lat)
   ...
   patchloop: do ipa = 1,csite%npatches
      ...
      csite%fsc_in_no_excess(ipa) = 0.0!JL
            ...
            if (theta(kroot) < theta_crit) then
               ...
               if (cpatch%phenology_status(ico) < 2) then
                  ...
                  csite%fsc_in(ipa) = csite%fsc_in(ipa)                                    &
                                     ...
                                     * f_labile(ipft)               
                  !JL  N from leaf drop that is not resorbed is going into fsn_in. 
                  !The resorbed N and will go into nstorage
                  csite%fsn_in(ipa) = csite%fsn_in(ipa)                                    &
                                    + (cpatch%nplant(ico) * cpatch%leaf_drop(ico)          &
                                    * f_labile(ipft) / c2n_leaf(ipft))                     &
                                    *(1.0 - resorption_factor)
                  ...
                  !------------------------------------------------------------------------!
                  ! Update nstorage pool for resorbed N from leaf drop   !JL               !
                  !------------------------------------------------------------------------! 
                     N_resorbed_phenology =  (cpatch%leaf_drop(ico)                        &
                                    * f_labile(ipft) / c2n_leaf(ipft))*resorption_factor
...
;               cpatch%leaf_drop(ico) = (1.0 - retained_carbon_fraction) * delta_bleaf
              ...
              !JL 52% of N from leaf drop is going into fsn_in. The rest is being resorbed
              ! and going into Nstorage.F_labile is set to  1
               csite%fsn_in(ipa) = csite%fsn_in(ipa)                                      &
                                 + cpatch%nplant(ico)                                     &
               ...
               ! Update nstorage pool for resorbed N from leaf drop   !JL               !
               !------------------------------------------------------------------------! 
                     N_resorbed_phenology = (delta_bleaf/ c2n_leaf(ipft)                 &
                                           * f_labile(ipft))*resorption_factor
                   ! now that we know how much N can be resorbed put it in N storage
print*, ' PHEN 19'  
                 call resorbed_N(ico,ipa,cpatch,csite,ipft,N_resorbed_phenology)
print*, ' PHEN 20'                 
...
subroutine assign_prescribed_phen(green_leaf_factor,leaf_aging_factor,dbh,height,pft       &
  ...
  ! print*,'BLEAF MAX', dbh2bl(dbh, pft) 
  ! output gives bleaf max, divide by leaf c2n to get N storage for full crown.
  ! = to last years full crown 
  !stop
...
subroutine resorbed_N(ico,ipa,cpatch,csite,ipft,N_resorbed_phenology)
  use nutrient_constants       , only: resorption_factor, nstorage_max_factor
  use allometry                , only: dbh2bl             
