module fuse_fiss_utils
         ...
         if ( csize < min_cohort_size(ipft) ) then
            ...
            csite%fsn_in(ipa) = csite%fsn_in(ipa) + cpatch%nplant(ico)                     &
                              * ( f_labile(ipft) * cpatch%balive(ico) / c2n_leaf(ipft)     &
                                + cpatch%nstorage(ico))  ! JL changed bstorage/c2n to nstorage
   ...
   subroutine split_cohorts(cpatch, green_leaf_factor, lsl)
      ...
      do ico = 1,cpatch%ncohorts
         ...
         ! flushed.                                                                        !
         !---------------------------------------------------------------------------------! 
       !  stai = cpatch%nplant(ico) * cpatch%balive(ico) * green_leaf_factor(ipft)          & !JL BUG IDENTIFIED BY XIANGTAO 
       ...
       !       * cpatch%sla(ico) + cpatch%wai(ico)
         stai = cpatch%nplant(ico) * cpatch%balive(ico) * green_leaf_factor(ipft)          &
              *1.0 / ( 1.0 + q(ipft) + qsw(ipft) * cpatch%hite(ico) )                 &
              * cpatch%sla(ico) + cpatch%wai(ico)
...
!******************************************************** JL,XXT
 ...
 cpatch%excess_carbon(idt)             = cpatch%excess_carbon(isc)
 cpatch%excess_carbon_fixer (idt)             = cpatch%excess_carbon_fixer (isc)
 cpatch%nitrogen_supply_fixer(idt)             = cpatch%nitrogen_supply_fixer(isc)
 cpatch%nitrogen_supply(idt)             = cpatch%nitrogen_supply(isc)
 cpatch%shadow_N_uptake(idt)             = cpatch%shadow_N_uptake(isc)
 cpatch%actual_nitrogen_uptake(idt)             = cpatch%actual_nitrogen_uptake(isc)
 cpatch%actual_nitrogen_uptake_fixer(idt)             = cpatch%actual_nitrogen_uptake_fixer(isc)
 cpatch%fixation_demand (idt)             = cpatch%fixation_demand (isc)
 cpatch%N_fixation(idt)             = cpatch%N_fixation(isc)
     ...
     !********************************************************
      ...
      cpatch%nstorage(idt)             = cpatch%nstorage(isc)!JL!
      ...
      enddo
!JL!
    ! do imonth = 1,FSN_ndays_to_avg                                  !JL!
     !    cpatch%N_limitation_factor(imonth,idt)         = cpatch%N_limitation_factor(imonth,isc)
      !enddo
!JL!
   !   cpatch%N_limitation_factor_bar(idt)              = cpatch%N_limitation_factor_bar(isc)!JL!
     ...
     cpatch%fsn_fixer(idt)             = cpatch%fsn_fixer(isc) !JL!
  cpatch%carbon_balance(idt)                  = cpatch%carbon_balance(isc) !JL!
  cpatch%N_uptake_pot(idt)                  = cpatch%N_uptake_pot(isc) !JL!
  cpatch%nitrogen_uptake(idt)                  = cpatch%nitrogen_uptake(isc) !JL!
  cpatch%carbon_balance_pot(idt)                  = cpatch%carbon_balance_pot(isc) !JL! 
  cpatch%nitrogen_supply_test(idt)                  = cpatch%nitrogen_supply_test(isc) !JL!
 cpatch%nitrogen_supply_test2(idt)                  = cpatch%nitrogen_supply_test2(isc) !JL!  
      ...
      cpatch%excess_carbon(idt)        = cpatch%excess_carbon(isc) !JL,XXT
     ...
     !    cpatch%mmean_nitrogen_supply   (idt) = cpatch%mmean_nitrogen_supply        (isc)!JL!
      !   cpatch%mmean_N_uptake_pot     (idt) = cpatch%mmean_N_uptake_pot        (isc)!JL!
...
!***********************************************************************************************JL,XXT
cpatch%excess_carbon(recc)  = ( cpatch%nplant(recc) * cpatch%excess_carbon(recc)               &
                             + cpatch%nplant(donc) * cpatch%excess_carbon(donc) ) *newni
cpatch%excess_carbon_fixer(recc)  = ( cpatch%nplant(recc) * cpatch%excess_carbon_fixer(recc)               &
                             + cpatch%nplant(donc) * cpatch%excess_carbon_fixer(donc) ) *newni
cpatch%nitrogen_supply_fixer(recc)  = ( cpatch%nplant(recc) * cpatch%nitrogen_supply_fixer(recc)               &
                             + cpatch%nplant(donc) * cpatch%nitrogen_supply_fixer(donc) ) *newni
cpatch%nitrogen_supply(recc)  = ( cpatch%nplant(recc) * cpatch%nitrogen_supply(recc)               &
                             + cpatch%nplant(donc) * cpatch%nitrogen_supply(donc) ) *newni
cpatch%shadow_N_uptake(recc)  = ( cpatch%nplant(recc) * cpatch%shadow_N_uptake(recc)               &
                             + cpatch%nplant(donc) * cpatch%shadow_N_uptake(donc) ) *newni
cpatch%actual_nitrogen_uptake(recc)  = ( cpatch%nplant(recc) * cpatch%actual_nitrogen_uptake(recc)               &
                             + cpatch%nplant(donc) * cpatch%actual_nitrogen_uptake(donc) ) *newni
cpatch%actual_nitrogen_uptake_fixer(recc)  = ( cpatch%nplant(recc) * cpatch%actual_nitrogen_uptake_fixer(recc)               &
                             + cpatch%nplant(donc) * cpatch%actual_nitrogen_uptake_fixer(donc) ) *newni
cpatch%fixation_demand(recc)  = ( cpatch%nplant(recc) * cpatch%fixation_demand(recc)               &
                             + cpatch%nplant(donc) * cpatch%fixation_demand(donc) ) *newni
cpatch%N_fixation(recc)  = ( cpatch%nplant(recc) * cpatch%N_fixation(recc)               &
                             + cpatch%nplant(donc) * cpatch%N_fixation(donc) ) *newni
!JL , note FSN_fixer follows fsn 
!cpatch% fsn_fixer(recc)  = (cpatch%nplant(recc) * cpatch%fsn_fixer(recc)               &
 !                            + cpatch%nplant(donc) * cpatch%fsn_fixer(donc) ) *newni
!******************************************************************************************************************
!
...
!JL!
      cpatch%nstorage(recc)  = ( cpatch%nplant(recc) * cpatch%nstorage(recc)               &
                               + cpatch%nplant(donc) * cpatch%nstorage(donc) ) * newni
!JL!
...
!JL,XXT
!cpatch%excess_carbon(recc) = newni                                                &
 !                           * ( cpatch%nplant(recc) * cpatch%excess_carbon(recc)        &
  !                            + cpatch%nplant(donc) * cpatch%excess_carbon(donc) )
!JL,XXT
...
!JL! 
!do imon = 1,FSN_ndays_to_avg-1
 !        cpatch%N_limitation_factor(imon,recc)     = ( cpatch%N_limitation_factor(imon,recc) * cpatch%nplant(recc)           &
  !                                  + cpatch%N_limitation_factor(imon,donc) * cpatch%nplant(donc) ) * newni 
!end do

!cpatch%N_limitation_factor(FSN_ndays_to_avg,recc)     = ( cpatch%N_limitation_factor(FSN_ndays_to_avg,recc) * cpatch%nplant(recc)                  & !JL!
 !                              + cpatch%N_limitation_factor(FSN_ndays_to_avg,donc) * cpatch%nplant(donc) ) * newni
!cpatch%N_limitation_factor_bar(recc)     = ( cpatch%N_limitation_factor_bar(recc) * cpatch%nplant(recc)                  &
  !                             + cpatch%N_limitation_factor_bar(donc) * cpatch%nplant(donc) ) * newni
!JL!
      ...
      cpatch%fsn_fixer(recc)          = ( cpatch%fsn_fixer(recc)      * cpatch%lai(recc)               &
                                  + cpatch%fsn_fixer(donc)      * cpatch%lai(donc) ) * newlaii
!JL!
!cpatch%carbon_balance(recc)          = ( cpatch%carbon_balance(recc)      * cpatch%lai(recc)               &
 !                                 + cpatch%carbon_balance(donc)      * cpatch%lai(donc) ) * newlaii  
!JL!
!JL!
!cpatch%N_uptake_pot(recc)          = ( cpatch%N_uptake_pot(recc)      * cpatch%lai(recc)               &
 !                                 + cpatch%N_uptake_pot(donc)      * cpatch%lai(donc) ) * newlaii  
!JL!
...
!JL!
!cpatch%mmean_nitrogen_supply        (recc) = ( cpatch%mmean_nitrogen_supply(recc)           &
 !                                                * cpatch%nplant(recc)                     &
  !                                               + cpatch%mmean_nitrogen_supply(donc)           &
   !                                              * cpatch%nplant(donc) ) * newni
!JL!
!cpatch%mmean_N_uptake_pot        (recc) = ( cpatch%mmean_N_uptake_pot(recc)           &
 !                                                * cpatch%nplant(recc)                     &
  !                                               + cpatch%mmean_N_uptake_pot(donc)           &
   !                                              * cpatch%nplant(donc) ) * newni
   ...
   subroutine fuse_2_patches(csite,donp,recp,mzg,mzs,prss,lsl,ntext_soil,green_leaf_factor &
      ...
      csite%N_leached(recp) = newareai *                                          &
                                     ( csite%N_leached(donp) * csite%area(donp)   &
                                     + csite%N_leached(recp) * csite%area(recp) )!JL!
      csite%N_gas_loss(recp) = newareai *                                          &
                                     ( csite%N_gas_loss(donp) * csite%area(donp)   &
                                     + csite%N_gas_loss(recp) * csite%area(recp) )!JL


csite%fsc_in_no_excess (recp) = newareai *                                          &
                                     ( csite%fsc_in_no_excess (donp) * csite%area(donp)   &
                                     + csite%fsc_in_no_excess (recp) * csite%area(recp) )!JL!

      csite%fast_soil_N(recp)        = newareai *                                          &
                                     ( csite%fast_soil_N(donp)        * csite%area(donp)   &
                                     + csite%fast_soil_N(recp)        * csite%area(recp) )
     
    csite%dist_fast_soil_N(recp)        = newareai *                                          &
                                     ( csite%dist_fast_soil_N(donp)        * csite%area(donp)   &
                                     + csite%dist_fast_soil_N(recp)        * csite%area(recp) ) !JL!
      
     csite%dist_structural_soil_C(recp)        = newareai *                                          &
                                     ( csite%dist_structural_soil_C(donp)        * csite%area(donp)   &
                                     + csite%dist_structural_soil_C(recp)        * csite%area(recp) ) !JL!
      ...
      if (imoutput > 0 .or. iqoutput > 0) then
         ...
         csite%mmean_Af_decomp(recp)       = newareai                                      &
                                           * ( csite%mmean_Af_decomp(donp)                 &
                                             ...
                                             * csite%area(recp) )
         !JL!
	 csite%mmean_fsn_in(recp)       = newareai                                         &
                                           * ( csite%mmean_fsn_in(donp)                    &
                                             * csite%area(donp)                            &
                                             + csite%mmean_fsn_in(recp)                    &
                                             * csite%area(recp) )	
	!JL!
	 csite%mmean_ssc_in(recp)       = newareai                                         &
                                           * ( csite%mmean_ssc_in(donp)                    &
                                             * csite%area(donp)                            &
                                             + csite%mmean_ssc_in(recp)                    &
                                             * csite%area(recp) )								     		
					     			  						     
	!JL!
	 csite%mmean_total_plant_nitrogen_uptake(recp)       = newareai                                         &
                                           * ( csite%mmean_total_plant_nitrogen_uptake(donp)                    &
                                             * csite%area(donp)                            &
                                             + csite%mmean_total_plant_nitrogen_uptake(recp)                    &
                                             * csite%area(recp) )							     
