...
subroutine reproduction(cgrid, month)
   ...
   use pft_coms           , only : recruittype           & ! structure
                                 ...
                                 , c2n_recruit,c2n_storage           & ! intent(in)
                                 , seed_rain             & ! intent(in)
                                 , include_pft           & ! intent(in)
                                 , include_pft_ag        & ! intent(in)
                                 , qsw                   & ! intent(in)
                                 , q,c2n_leaf                     & ! intent(in)
                                 , sla                   & ! intent(in)
                                 , hgt_min               & ! intent(in)
   ...
   real                                :: elim_nplant
   real                                :: elim_lai
   real, dimension(n_pft) :: full_flush
   !----- Saved variables -----------------------------------------------------------------!
   logical          , save             :: first_time = .true.
   !---------------------------------------------------------------------------------------!
real :: nbefore, nafter,pn1,pn2

full_flush = 1.
nbefore=0.;nafter=0.;pn1=0.;pn2=0.
   !---------------------------------------------------------------------------------------!
      ...
      siteloop: do isi = 1,cpoly%nsites
         ...
         patchloop: do ipa = 1,csite%npatches
            ...
            do ico = 1, cpatch%ncohorts
                     nbefore = nbefore + cpatch%nplant(ico)*cpatch%bseeds(ico)*csite%area(ipa)&
                          *(1.0-seedling_mortality(cpatch%pft(ico)))/c2n_recruit(cpatch%pft(ico))
                     pn1=pn1+csite%area(ipa)*cpatch%nplant(ico)*(cpatch%balive(ico)/c2n_leaf(cpatch%pft(ico))  &
                          +cpatch%bdead(ico)/c2n_stem(cpatch%pft(ico))+cpatch%nstorage(ico)+cpatch%bseeds(ico) &
                          *(1.0-seedling_mortality(cpatch%pft(ico)))/c2n_recruit(cpatch%pft(ico)))!JL added nstorage
            enddo
            !---- This time we loop over PFTs, not cohorts. -------------------------------!
            pftloop: do ipft = 1, n_pft
            nafter = nafter + csite%repro(ipft,ipa)*csite%area(ipa)/c2n_recruit(ipft)

               
               if( include_pft(ipft) .and. repro_scheme /= 0 ) then
...
!                     if (.false.)then
         ...
         update_patch_loop: do ipa = 1,csite%npatches
            ...
            do ico=1,cpatch%ncohorts
               pn2=pn2+csite%area(ipa)*cpatch%nplant(ico)*(cpatch%balive(ico)/c2n_leaf(cpatch%pft(ico)) &
                    +cpatch%bdead(ico)/c2n_stem(cpatch%pft(ico)) + cpatch%nstorage(ico))  !JL added nstorage
            enddo
